...
**Security Advisor finding — found and fixed this session**: Supabase's built-in Security Advisor (Dashboard → Advisors → Security, also runnable via `supabase db advisors --linked --type security`) reported **109 warnings, 0 errors, 4 info suggestions**. Not part of §1's table-RLS audit (which only checks tables, not function grants) — a different, real gap, closed via `supabase/migrations/0033-0035`:

- **`function_search_path_mutable` (7 functions)**: `generate_document_number`, `set_updated_at`, `requesting_role`, `has_permission`, `get_sample_size_plan`, `enforce_fg_stock_origin`, `prevent_stock_transactions_mutation` predated the `set search_path = public` convention every function since Phase 6 has followed. Fixed with `ALTER FUNCTION ... SET search_path = public` (no body change). **Confirmed gone** on re-run.
- **`anon_security_definer_function_executable` (42 functions)**: read `pg_proc.proacl` directly rather than guessing — every function had EXECUTE granted to `anon` explicitly via Supabase's project-level `ALTER DEFAULT PRIVILEGES`, separate from (and not fixed by revoking) the `PUBLIC` default grant. Fixed with `REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC` (0033) and, once the Advisor re-run showed the real cause, `... FROM anon` (0034). **Confirmed gone**: a direct `SELECT get_stock_report()` as `anon` now fails with `permission denied` (Postgres-level, not just the app's own `has_permission()` check) — real defense-in-depth, not just relying on every function remembering its internal check.
- **Real regression found and fixed during this same pass**: revoking `authenticated`'s direct EXECUTE on `has_permission()`/`requesting_role()`/`generate_document_number()` (0033) broke `create_purchase_order()` — it calls them via ordinary nested SQL, and (empirically, in this managed Postgres) that does NOT inherit the calling `SECURITY DEFINER` function's elevated privileges the way assumed. Caught by testing a real `create_purchase_order()` call as `authenticated` before considering this done, not by inspection alone. Reverted just those three grants (0035); confirmed working end-to-end afterward (a real PO created successfully), plus a broader regression check across `get_wip_stock`/`get_fg_stock`/`get_stock_adjustments`/`get_my_notifications`/`has_permission`. The other six functions in the same original list (`audit_trigger_fn`, `handle_new_auth_user`, `rls_auto_enable`, `enforce_fg_stock_origin`, `prevent_stock_transactions_mutation`, `set_updated_at`) stay revoked from `authenticated` — they're only ever invoked by Postgres's trigger/event-trigger machinery, never by a nested SQL call, so revoking them is safe and closes off a pointless direct-RPC surface.
- **Remaining 55 `authenticated_security_definer_function_executable` + the 4 info suggestions**: not a gap — this is the app's real, intentional RPC surface (every `confirm_*`/`create_*`/`get_*` function `authenticated` is *supposed* to call, each gated internally by its own `has_permission()` check). Revoking these would break the app; the Advisor can't see the internal check, so it flags them regardless. Accepted as-is.
- **`auth_leaked_password_protection` (1 warning, not SQL-fixable)**: attempted to enable "Prevent use of leaked passwords" (Auth → Sign In/Providers → Email) directly — blocked with the same message as PITR: **Pro Plan required** (HaveIBeenPwned checking is a paid Auth feature). Left off, consistent with the user's PITR decision above; the toggle is a two-click fix once the Pro decision is revisited.

**Final count: 109 → 56 → 1** (55 accepted-as-intentional + 1 Pro-plan-gated). Re-run `supabase db advisors --linked --type security` any time to confirm nothing has regressed.
