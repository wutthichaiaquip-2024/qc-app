...
## Phase 21 — Reports: สถานะ **เสร็จ (query functions + background export job เดินจริงและ verified end-to-end, UI ยังไม่ได้ทดสอบในเบราว์เซอร์)**

Scope decisions confirmed via AskUserQuestion before building: export formats = **real CSV + real PDF** (no `.xlsx` — stays clear of the `xlsx`/SheetJS CVEs already ruled out in Phase 3; PDF via `pdf-lib`, no headless browser); "background job" = a **genuine job queue** (not a synchronous request), since Storage is this same Supabase project's own API, not a third-party credential Phase 20 was missing.

Done:
- Migration `supabase/migrations/0027_reports.sql`: `get_stock_report()` (all zones/sites, not just WIP/FG), `get_qc_report(from, to)` (unions IQC/FG Inspection/OQC into one row-per-inspection-event shape, OQC's picking-level qty/part_no aggregated via a lateral join since Phase 15 has no per-lot split), `get_supplier_quality_report(from, to)` (NG rate per supplier from Phase 6/7 data — aggregate only, no CAR/claim tracking, confirmed out of scope before Phase 0). Forecast and Traceability deliberately reuse Phase 19's `get_forecast_accuracy()` and Phase 17's `get_lot_genealogy()` rather than duplicating them.
- `report_jobs` (PENDING/PROCESSING/DONE/FAILED queue) + `create_report_job()` (checked, `has_permission('reports','view')`) + `get_my_report_jobs()`, same permission-gated RPC pattern as every write/read since Phase 6.
- `get_report_export_data(report_type, filters)`: the **only** thing the worker calls — `revoke`d from `authenticated`, `grant`ed to `service_role` only. Locally simulates an ADMIN JWT claim via `set_config` (scoped to that one call) purely to satisfy the nested `has_permission()` checks inside `get_stock_report()` etc., since the worker runs via the service_role key with no real request/JWT context (same unconditional-compute shape as Phase 18's materialized-view refresh) — the actual authorization already happened once, for real, in `create_report_job()` at enqueue time.
- **The export worker is a real, deployed, tested Edge Function** — `supabase/functions/process-report-jobs/index.ts`, invoked every minute by `pg_cron`+`pg_net` (`process-report-jobs-every-minute`). Unlike Phase 20's email/LINE sender, this one only calls this same Supabase project's own Storage API (no third-party SMTP/LINE-style credential needed), so it actually is deployed and actually is exercised end-to-end in this environment: renders CSV (hand-written serializer) or a paginated PDF (`pdf-lib`, flattens the Traceability genealogy tree into Field/Value rows when the export target is a nested JSON object rather than a row array), uploads to the private `report-exports` bucket at `<requested_by uuid>/<job id>.<ext>`, marks the job `DONE`/`FAILED`.
- Storage RLS on `report-exports` is **per-user**, not per-module like Phase 10/14's buckets (`(storage.foldername(name))[1] = auth.uid()::text`) — these are personal export files, not shared module data. Client downloads use `createSignedUrl()` with the user's own session, same as Phase 10's `item-documents`.
- **Verified against the real dev DB and the real deployed function, not just written**: built out a full temporary chain (site/location/supplier/item/lot/stock/PO/GR/IQC with a deliberate `qty_ng=10` of `qty_received=100`), created STOCK/PDF + QC/CSV + SUPPLIER_QUALITY/CSV + TRACEABILITY/CSV jobs, manually invoked the deployed function (not waiting on the cron minute) — all 4 went `PENDING → PROCESSING → DONE` with zero `FAILED`. Downloaded and read back every file: QC CSV showed the exact IQC row; Supplier Quality CSV showed `ng_rate_pct = 10` (10/100×100, matching the hand-computed expectation exactly); Stock PDF was a valid `%PDF-1.7` file; Traceability CSV correctly flattened the full genealogy tree (receiving → PO → supplier → IQC result). Confirmed the anon key alone can't read another user's export (`storage.objects` "not found", RLS-blocked). Confirmed `create_report_job()` rejects when `reports.view` is off (temporarily disabled, restored after). Confirmed `report_jobs` RLS: a second simulated user (via `set role authenticated` + a different `sub`, not just `set_config` alone — that runs as a superuser and bypasses RLS entirely) saw 0 rows, the real requester saw all 5. All test data, storage files, and audit_log entries cleaned up after (verified back to 0 rows).
...
- **PITR + Monitoring/Alerting**: checked directly in the Dashboard (logged in as the user). Org is on the Free Plan — real risk for production: free projects auto-pause after 1 week idle. PITR needs Pro ($25/mo+) plus its own add-on ($100/$200/$400 per month for 7/14/28-day retention); **user decided not to upgrade yet** (2026-09-02). Baseline monitoring (CPU/RAM/disk/slow queries/per-service health) is already free, no action needed; proactive alerting has no self-service page found, only paid Log Drains ($60/drain/mo, Pro-only). Full numbers in `PRODUCTION_RELEASE.md` §2.
- **Security Advisor finding, found and fixed**: 109 warnings (not part of §1's table-only RLS audit — function grants, a different layer) → 56 after `0033`-`0035` → 55 accepted-as-intentional (the app's real RPC surface, each gated internally by `has_permission()`) + 1 blocked by the same Free-plan limit as PITR (leaked-password-protection needs Pro). Fixed: 7 functions missing `set search_path`; 42 functions callable by `anon` (Supabase grants `anon` EXECUTE separately from `PUBLIC` by default — revoking `PUBLIC` alone didn't fix it, had to revoke `anon` explicitly too, confirmed via `pg_proc.proacl` inspection, not guessed). **Caught a real regression before calling it done**: revoking `authenticated` from `has_permission()`/`requesting_role()`/`generate_document_number()` broke `create_purchase_order()` (nested SQL calls don't inherit the caller's SECURITY DEFINER elevation the way assumed) — found by testing a real PO creation, reverted those 3 grants, re-verified. Full detail in `PRODUCTION_RELEASE.md` §2.
...
- `supabase/migrations/0033-0035` — Security Advisor fixes: search_path on 7 functions, revoke PUBLIC+anon execute from every function, restore the 3 grants (has_permission/requesting_role/generate_document_number) that broke real functionality when revoked
