...
- **Unplanned finding**: Supabase's Security Advisor (free, not part of §1's table-only RLS audit) reports 109 real warnings — mostly functions missing `set search_path` and some `SECURITY DEFINER` functions callable by `PUBLIC` that should be `authenticated`-only (a gap from earlier phases before the later "revoke public/anon, grant authenticated" pattern was consistently applied). Not fixed yet — see `PRODUCTION_RELEASE.md` §2 for the exact function names found.
